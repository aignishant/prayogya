# Architecture Decision Records — Prayoga
...
`CHANGELOG_PLAN.md`), or a decision inside a single day (that belongs in the day). A decision
inside a single *project* belongs in that project, not here — the forty projects are independent,
and this folder is the authoring repository only.
