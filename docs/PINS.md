...
| MCP specification | 2026-07-28 | 2026-09-12 | P02 d3 | Supersedes the row above's claim that "P02 cannot run it": that sentence was true of `google-adk[mcp]`'s `mcp<2` extra and stayed true of it, but P02 day 3 installs `mcp==2.2.0` directly with `uv add "mcp==2.2.0"`, exactly as ADR-0009 records for P01, and never touches the `google-adk` extra at all. The revision itself is unchanged; what was wrong was the inference drawn from it. |
